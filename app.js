...
const saluteContainer = document.querySelector(".hiTextcontainer");
const imageContainer = document.querySelector(".hiImgcontainer");
...
window.addEventListener("scroll", () => {
  ...
  saluteContainer.style.transform = `translateY(${scrollY * 0.1}px)`;
  imageContainer.style.transform = `translate(${scrollY * 0.4}px, ${
    scrollY * 0.7
  }px)`;
  authorName.style.transform = `translateX(calc(-100vh + ${scrollY}px))`;
  jobTitleContainer.style.backgroundPositionY = `${scrollY * 0.5}px`;
  jobTitle.style.transform = `translateX(calc(200vh - ${scrollY}px))`;
  projectsTitle.style.transform = `translateY(calc(300vh - ${scrollY}px))`;
  phones[0].style.transform = `translateX(calc(360vh - ${scrollY}px))`;
  laptop.style.transform = `translateX(calc(-465vh + ${scrollY}px))`;
  phones[1].style.transform = `translateX(calc(585vh - ${scrollY}px))`;
});
